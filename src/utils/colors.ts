...
const DEFAULT_SHADE_COLOR: RGBA = [0, 0, 0, 0.6];
...
export default class ColorParser {
    static parseRgba(rgbaColor: string): RGBA | null {
        const m = rgbaColor.match(/rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,?\s*(\d+\.?\d*)?\s*\)/);
        return m ? ([m[1], m[2], m[3], m[4] || 1].map(Number) as RGBA) : null;
    }
    ...
    shade(color: RGBA | string, value: number) {
        const _color = typeof color === 'string' ? this.toRgba(color, DEFAULT_SHADE_COLOR) : color;
